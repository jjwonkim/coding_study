...
class ExcelPDFChatbot:
    def __init__(
        ...
        # [지시사항 1-B] 생성한 코드에서 에러가 발생할 경우 다시 코드를 생성하도록 로직을 수정하세요.
        # Hint 1. self.graph.add_conditional_edges() 메서드를 사용합니다.
        # Hint 2. 지시사항 1-A를 따라 구현할 self._check_generated_code_error 메서드도 활용합니다.
        #################################################
        # 아래 간선을 지시사항과 힌트를 참고하여 조건부 간선으로 바꿔보세요.
    ...
    def query(self, state: State):
        ...
        # [지시사항 1-C] State에 에러 메시지가 있다면, 이 내용을 프롬프트에 추가합니다.
        # Hint. 생성한 코드를 "assistant" 역할의 텍스트로,
        # 에러 메시지와 수정 요청을 "human" 역할의 프롬프트로 순서대로 추가해보세요.
        ###########################################
        if isinstance(state["data"], list) and len(state["data"]) > 0:
            ...
            for c, d in zip(state["code"], state["data"]):
                message_with_data_info.append(("assistant", c))
                message_with_data_info.append(("human", f'{d}. 이전에 작성한 코드에 에러가 있습니다. 에러를 수정해 주세요.'))

            # raise NotImplementedError("query 메서드의 지시사항 1-C를 구현해주세요.")
        ###########################################
    ...
    def plot_graph(self, state: State):
        ...
        if isinstance(state["data"], list) and len(state["data"]) > 0:
            ...
            for c, d in zip(state["code"], state["data"]):
                message_with_data_info.append(("assistant", c))
                message_with_data_info.append(("human", f'{d}. 이전에 작성한 코드에 에러가 있습니다. 에러를 수정해 주세요. {question}'))

            # raise NotImplementedError(
            #     "plot_graph 메서드의 지시사항 1-C를 구현해주세요."
            # )
        ###########################################
        prompt_with_data_info = ChatPromptTemplate.from_messages(message_with_data_info)
    ...
    def _check_generated_code_error(self, state: State) -> str:
        ...
        #############################
        # [지시사항 1-A] 코드 실행 결과에 에러가 있는지 판단하는 로직을 작성하세요.
        # 에러가 5번 초과로 발생할 경우, 에러 여부와 무관하게 "False"를 반환하세요.
        # Hint. utils.run_code() 함수에서, 생성한 코드에서 에러가 발생할 경우 어떤 텍스트를 반환하는지 확인합니다.
        # raise NotImplementedError("지시사항 1-A를 구현해주세요.")
        
        if len(code_exec_result_list) > 5:
            result = "False" # 너무 많이 실행했으므로 더 이상 실행하지 않음.
        elif 'Error:' in code_exec_result_list[-1]:
            result = "True" # 에러가 있으므로 재 생성
