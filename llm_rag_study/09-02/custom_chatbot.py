...
class SamsungCatalogueChatbot:
    ...
    def route_question(self, state: State):
        ...
        # Hint 2. catalogue의 특징은 self.catalog_description 저장되어 있습니다.
        #################################################
        # route_system_message = "YOUR SYSTEM PROMPT HERE"
        route_system_message = (
            "당신은 사용자의 질문에 RAG, 웹 검색 중 어떤 것을 활용할 수 있는지 결정하는 전문가입니다."
            f"{self.catalog_description} 과 관련된 질문이라면 rag 를 활용하세요. \n"
            "그 외의 질문이라면 web_search 로 충분합니다. \n"
            f"주어진 질문에 맞춰 `rag`, `web_search` 중 하나를 선택하세요. \n"
